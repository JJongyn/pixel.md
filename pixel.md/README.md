...
This exact integration is available as a [runnable Vite example](examples/vite/README.md). `<pixel-chat-effect>` adds a pixel treatment around your input; your app still owns the textarea, messages, and send behavior. Update the indicator's `state`, `label`, and `detail` from your real agent state.
